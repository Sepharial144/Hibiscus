...
void Configuration::setConfigurationPath(const char* config_path,
    const char* name)
{
    m_configPath = config_path;
    m_configName = name;
...
interface Configuration::getInterfaceValue(const char* interface_prop)
{
    ...
    for (auto i = 0u; i < sizeInterface; ++i) {
        ...
        if (name == interface_prop) {
            return interface { interface_prop, iface[i]["top"].as<int32_t>(),
...
{
    return resolution { m_config["resolution"]["width"].as<int32_t>(),
